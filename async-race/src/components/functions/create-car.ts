...
import { Car, getCars } from '../../api-requests/get-cars';
...
class NewCar {
  ...
  async createCar() {
    ...
    const buttonNext = document.querySelector('.btn_next');
    const garageName = document.querySelector('.page-name');
    if (buttonNext instanceof HTMLButtonElement && garageName != null) {
      const pageNumber = buttonNext.dataset.page;
      const dates = await getCars();
      garageName.textContent = `Garage(${dates.length})`;
      const pages = Math.ceil(dates.length / 7);
      if (pages === Number(pageNumber)) {
