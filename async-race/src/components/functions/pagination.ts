import { Car, getCars, Cars } from '../../api-requests/get-cars';
...
export async function pagination() {
  ...
  if (buttonNext instanceof HTMLButtonElement) {
    const numberPage = buttonNext.dataset.page;
    count = Number(numberPage);
  }
  const rows = 7;
  function displayPage(arrData: Cars, perPage: number, page: number) {
    ...
    if (garage != null) {
      ...
      paginationData.forEach((car: Car) => {
  ...
  function paginationBtn() {
    ...
    if (buttonNext instanceof HTMLButtonElement && buttonPrev instanceof HTMLButtonElement) {
      buttonNext.addEventListener('click', (e: Event) => {
        e.preventDefault;
        count++;
        buttonNext.dataset.page = `${count}`;
        buttonPrev.dataset.page = `${count}`;
        buttonPrev.removeAttribute('disabled');
        displayPage(dates, rows, count);
      });

      buttonPrev.addEventListener('click', (e: Event) => {
        e.preventDefault;
        count--;
        buttonNext.dataset.page = `${count}`;
        buttonPrev.dataset.page = `${count}`;
      ...
      if (page != null) page.textContent = `page #${count}`;
      if (count === 1) displayPage(dates, rows, count);
    }
  }
